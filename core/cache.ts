...
var theCache: { [ name: string ]: any } = {};

var strip = function(text: string) {
		...
		getText = function(url: string, sync: boolean, load: (data: any) => void) {
			if ((url.indexOf('http:') === 0) || (url.indexOf('https:') === 0)) {
				...
				if (isAmd) {
					...
					req([fsModule], function(fs: any) {
						fs.readFile(url, { encoding: 'utf-8' }, function(err: any, content: string) {
							if (!err) {
								load(content);
							}
							else {
					...
					req('fs').readFile(url, { encoding: 'utf-8' }, function(err: any, content: string) {
						if (!err) {
							load(content);
						}
						else {
...
cache = (() => {
	var cache: any = function( /*String||Object*/ module: any, /*String*/ url: string, /*String||Object?*/ value: any) {
		// summary:
		...
		if (typeof val === 'string') {
			...
			//not exist, fetch it.
			if (!(key in theCache)) {
				getText(key, true, function(text: string) {
					theCache[key] = text;
				});
			}
