...
export var domProcessor: typeof _domProcessor = _domProcessor
export var textProcessor: typeof _textProcessor = _textProcessor
...
export class Template extends Properties {
	...
	compileTextSync () {
		if (this.compiledTextVersion) {
			...
			return textProcessor.compileText(this.text, true);
		}
	}
	compileText (sync: boolean): any {
		...
		if (sync) {
			...
			if (!result) {
				//Do some processing
				result = textProcessor.compileText(this.text, sync);
				if (sync) {
